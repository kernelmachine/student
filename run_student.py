    ...
    for epoch in range(training_epochs):
        ...
        if epoch % display_step == 0 and epoch > 0:
            ...
            if X_dev is not None:
                ...
                n_dev, _ = X_dev.shape
                if network_architecture['n_labels'] > 0:
                    dev_predictions = predict_labels(model, network_architecture, X_dev, C_dev)
...
def infer_categorical_covariate(model, network_architecture, X, Y):
    n_items, vocab_size = X.shape
    ...
    if n_covariates == 1:
        for i in range(n_items):
            ...
            if Y is not None:
                ...
                Y_i = None
            losses = model.get_losses(X_i, Y_i, C_i)
...
def predict_labels(model, network_architecture, X, C):
    n_items, vocab_size = X.shape
    predictions = np.zeros(n_items, dtype=int)
    ...
    for i in range(n_items):
        ...
        z, y_recon = model.predict(X_i, C_i)
