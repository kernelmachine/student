...
def main():
    ...
    preprocess_data(train_infile, test_infile, output_dir, train_prefix, test_prefix, min_doc_count, max_doc_freq, vocab_size, stopwords, keep_num, keep_alphanum, strip_html, min_length, label_name=label_name, bigrams=bigrams)


def preprocess_data(train_infile, test_infile, output_dir, train_prefix, test_prefix, min_doc_count=0, max_doc_freq=1.0, vocab_size=None, stopwords=None, keep_num=False, keep_alphanum=False, strip_html=False, min_length=3, label_name=None, output_plaintext=False, bigrams=False):
    ...
    for i, item in enumerate(all_items):
        ...
        text = item['text']
        tokens = tokenize(text, strip_html, keep_numbers=keep_num, keep_alphanum=keep_alphanum, min_length=min_length, stopwords=stopword_set, bigrams=bigrams)
